...
def main():
    ...
    EPOCHS = 2
    BATCH_SIZE = 1
    dataset = np.load('../datasets/PCG/circor_final/springer_circor_dataset.npy', allow_pickle=True)
    ...
    print(f'Discarded {len(features) - len(good_indices)} sounds.')

    # 10-fold cross validation

    experiment_logger = PCGExperimentLogger(path='../results/unet/circor', name='unet', number_folders=number_folders)
    #
    model = unet_pcg(nch, patch_size)
    model.compile(optimizer=Adam(learning_rate=learning_rate), loss='categorical_crossentropy',
                  metrics=['categorical_accuracy'])
    model.save_weights('random_init_unet')
    savedir = 'tmpCC_Circor_' + datetime.datetime.now().strftime("%Y_%m_%d")
    #
    ...
    for fold in range(number_folders):
        ...
        print("f/l shape ", features_train.shape, " ", labels_train.shape)
        ...
        dataset_ = np.array(dataset_np, dtype=object)
        labels_ = dataset_[:, 1]
